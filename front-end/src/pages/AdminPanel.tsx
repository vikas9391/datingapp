...
const AdminPanelContent: React.FC = () => {
  ...
  // Desktop sidebar: open by default only on lg+ screens
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex relative">
      ...
      {/* ── SIDEBAR (desktop always visible if sidebarOpen; mobile as overlay drawer) ── */}
      <aside
      ...
      {/* ── MAIN CONTENT ── */}
      {/* On mobile: no margin (sidebar is overlay). On desktop (lg+): margin matches sidebar state */}
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ease-in-out ml-0 ${sidebarOpen ? 'lg:ml-64' : 'lg:ml-0'}`}>
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                {/* Mobile: always show hamburger. Desktop: show only when sidebar is collapsed */}
                <button
                  onClick={() => { setSidebarOpen(true); setDrawerOpen(true); }}
                  className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 shadow-sm shrink-0
                    lg:hidden
                ...
                {/* Desktop only: show shield/open button when sidebar is collapsed */}
                {!sidebarOpen && (
                  <button
        ...
        {/* Shows the 5 most common tabs; tap "More" to open the full sidebar drawer */}
